...
void WebPageDownloader::setUrl(const QUrl &url)
{
    m_url = url;
}

void WebPageDownloader::setFileName(const QString &fileName)
{
    m_fileName = fileName;
    m_resourcePath = fileName + ".res";
}

void WebPageDownloader::start()
{
    if (isRunning())
        return;
...
void WebPageDownloader::abort()
{
    ...
    foreach (WebFileDownloader *downloader, m_resources)
        downloader->abort();
...
QString WebPageDownloader::errorString() const
{
    switch (m_error)
    {
    case NoError:
...
void WebPageDownloader::resource_finished()
{
    m_resources.removeAll(qobject_cast<WebFileDownloader *>(sender()));
    updateProgress();
}

void WebPageDownloader::webpage_loadFinished(bool ok)
{
...
QString WebPageDownloader::addUrlToDownloader(const QString &urlstr, const QString &ext)
{
    ...
    WebFileDownloader *downloader = new WebFileDownloader(m_network, this);
...
void WebPageDownloader::updateProgress()
{
