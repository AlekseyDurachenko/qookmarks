...
void WebPageScreenshot::setUrl(const QUrl &url)
{
    m_url = url;
}

void WebPageScreenshot::setScreenshotSize(const QSize &size)
{
    m_screenshotSize = size;
}

void WebPageScreenshot::start()
{
    if (isRunning())
        return;
...
QString WebPageScreenshot::errorString() const
{
    switch (m_error)
    {
    case NoError:
...
void WebPageScreenshot::webpage_loadFinished(bool ok)
{
