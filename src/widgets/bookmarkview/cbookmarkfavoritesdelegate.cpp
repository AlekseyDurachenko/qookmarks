...
CBookmarkFavoritesDelegate::CBookmarkFavoritesDelegate(QObject *parent) :
    QItemDelegate(parent)
{
}

void CBookmarkFavoritesDelegate::paint(QPainter *painter,
        const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if (item->data().isFavorite())
    {
        painter->drawPixmap(option.rect.left() + (option.rect.width()-size)/2,
                            ...
                            QIcon(":/icons/bookmark-favorites.png")
